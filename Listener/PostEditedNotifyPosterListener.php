 ...
 */
class PostEditedNotifyPosterListener implements EventSubscriberInterface
{
    /**
    ...
    {
        return [
            DizkusEvents::POST_UPDATE => ['notifyPoster']
        ];
    }
    ...
    private function sendPosterPostNotification(PostEntity $post, $message)
    {
        ...
        $email = \Swift_Message::newInstance()
            ->setSubject($this->translator->__f('Your post in topic %topic on %website was modified', ['%topic'=> $post->getTopic()->getTitle(), '%website' => $sitename]))
            ...
            ->setBody(
                $this->twig->render(
                    '@ZikulaDizkusModule/Email/post.edited.notify.poster.html.twig',
            ...
            ->addPart(
                $this->twig->render(
                    '@ZikulaDizkusModule/Email/post.edited.notify.poster.txt.twig',
                    ['post' => $post, 'message' => $message]
                ),
                'text/plain'
            );
