...
function smarty_function_adminlink($params, &$smarty) 
{
    ...
    if (SecurityUtil::checkPermission(0, 'Dizkus::', "::", ACCESS_ADMIN)) { 
        ...
        if ($params['type'] == 'category') {
            return '<a href="' . DataUtil::formatForDisplay(ModUtil::url('Dizkus', 'admin', 'category', array('cat_id'=>(int)$params['id']))) . '">[' . DataUtil::formatForDisplay(__('Edit category title', $dom)) . ']</a>';
        } elseif ($params['type'] == 'forum') {
            return '<a href="' . DataUtil::formatForDisplay(ModUtil::url('Dizkus', 'admin', 'forum', array('forum_id' => (int)$params['id']))) . '">['.DataUtil::formatForDisplay(__('Edit forum', $dom)) . ']</a>';
        }
    }
