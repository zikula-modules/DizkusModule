...
//FC            //ORDER BY t.sticky DESC, p.post_time DESC";
    ...
    $topics_start = $start; // FIXME is this still correct?
...
function Dizkus_userapi_updatepost($args)
{
    ...
    if (!($pn_uid == $poster_id) &&
        ...
        return showforumerror(getforumerror('auth_mod', $forum_id, 'forum', __('Sorry! You do not have authorisation to moderate this category or forum.', $dom)), __FILE__, __LINE__);
    }
...
function Dizkus_userapi_get_viewip_data($args)
{
    ...
    if ($result->EOF) {
        ...
        return showforumerror(__('Error! No such user or post was found in the database.', $dom), __FILE__, __LINE__);
    } else {
        $row = $result->GetRowAssoc(false);
    }
    dzkCloseDB($result);
...
function Dizkus_userapi_movetopic($args)
{
    ...
    if ($oldforum_id <> $forum_id) {
        ...
        if ($shadow == true) {
            ...
            $subject = '***' . DataUtil::formatForDisplay(__('The original posting has been moved to here.', $dom)) . ': ' . $topic_title;
    ...
    if ($result->RecordCount()>0) {
        for (; !$result->EOF; $result->MoveNext())
        {
            ...
            if (SecurityUtil::checkPermission('Dizkus::', "$cat_id:$forum_id:", ACCESS_READ, $pn_uid)) {
                $email['name']    = pnUserGetVar('uname', $pn_uid);
