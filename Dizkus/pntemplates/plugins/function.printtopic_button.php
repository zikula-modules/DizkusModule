...
function smarty_function_printtopic_button($params, &$smarty)
{
    ...
    if (allowedtoreadcategoryandforum($params['cat_id'], $params['forum_id'])) {
        ...
        return '<a class="dzk_img printlink" title="' . DataUtil::formatForDisplay(__('Print topic', $dom)) . '" href="' . DataUtil::formatForDisplay(pnModURL('Dizkus', 'user', 'print', array('topic' => $params['topic_id']))) . '">' . DataUtil::formatForDisplay(__('Print topic', $dom)) . '</a>';
    }
