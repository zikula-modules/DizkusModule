...
function smarty_function_topicpager($params, &$smarty)
{
    ...
    if ( $add_prevnext ) {
        if ( $on_page > 1 ) {
            $page_string = '<a href="' . DataUtil::formatForDisplay(pnModURL('Dizkus', 'user', 'viewtopic', array('topic' => $topic_id, 'start' => ( $on_page - 2 ) * $per_page ))) . '">-1</a>] ' . $page_string;
            $add_prev_set = true;
        }
        if ( $on_page > 10 ) {
            $page_string = '<a href="' . DataUtil::formatForDisplay(pnModURL('Dizkus', 'user', 'viewtopic', array('topic' => $topic_id, 'start' => ( $on_page - 11) * $per_page ))) . '">-10</a> ' . $page_string;
            $add_prev_set = true;
        }
        if ( $on_page > 100 ) {
