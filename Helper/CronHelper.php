                                        ...
                                        if (empty($original_topic_id)) {
                                            if (empty($pop3conn['matchstring']) || 0 != preg_match($pop3conn['matchstring'], $subject)) {
                                                ...
                                                if (!empty($replyto)) {
                                                    ...
                                                    $topic = $this->entityManager->getRepository('Zikula\DizkusModule\Entity\TopicEntity')->findOneBy(['msgid', $replyto]);
                                                    if (!isset($topic)) {
                                                        ...
                                                        list($start, $post_id) = ModUtil::apiFunc($this->name, 'user', 'storereply', [
                                                                    'topic_id'         => $topic_id,
                                                                    'message'          => $message,
                                                                    'attach_signature' => 1,
                                                                    'subscribe_topic'  => 0,
                                                ...
                                                if (empty($replyto)) {
                                                    ...
                                                    $topic_id = ModUtil::apiFunc($this->name, 'user', 'storenewtopic', [
                                                                'subject'          => $subject,
                                                                'message'          => $message,
